...
export default function App() {
  ...
  // Update theme class on mount / change
  useEffect(() => {
    if (darkMode) {
      document.documentElement.classList.add("dark");
      writeLocalStorageValue(STORAGE_KEYS.theme, "dark");
    } else {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-neutral-950 font-sans text-slate-800 dark:text-neutral-200 transition duration-150 flex flex-col md:flex-row antialiased">
      
      {/* 1. Lateral Left Sidebar Navigation */}
      <aside className="w-full md:w-64 bg-slate-900 dark:bg-neutral-900 border-r border-[#e2e8f0]/10 text-zinc-300 p-5 flex flex-col justify-between shrink-0 font-sans shadow-sm">
        <div className="space-y-6">
          ...
          <div className="flex items-center gap-3 pb-4 border-b border-white/10">
            ...
            <div>
              <h1 className="text-xs font-black text-white tracking-widest uppercase mb-0.5 leading-none font-display">{APP_NAME}</h1>
              <span className="text-[9px] text-zinc-400 uppercase tracking-widest font-bold block">v{APP_VERSION} · Atelier & Restitutions</span>
            </div>
          </div>
            ...
            {[
              ...
              return (
                <button
                  ...
                  className={`w-full p-2.5 px-3 rounded-lg font-bold text-xs flex items-center justify-between transition-all duration-100 ${
                    isSel 
        ...
        <div className="pt-4 border-t border-white/5 space-y-4">
          <div className="flex items-center justify-between text-xs">
            ...
            <button 
              ...
              className="p-1 px-2.5 bg-white/5 hover:bg-white/10 rounded-md text-zinc-400 hover:text-white transition"
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="p-4 bg-white dark:bg-neutral-900 border-b border-slate-200 dark:border-neutral-800 flex flex-col sm:flex-row justify-between items-center gap-4 shrink-0 shadow-sm z-10">
          ...
          <div className="relative w-full sm:max-w-md">
            ...
            <input 
              type="text" 
              className="w-full bg-slate-50 dark:bg-neutral-950 border border-slate-200 dark:border-neutral-800 rounded-lg p-2.5 pl-9 pr-8 text-xs font-semibold focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500/10 placeholder-zinc-400 dark:text-neutral-100" 
            ...
            {globalSearchTerm && (
              <button 
                ...
                className="absolute right-3 top-3 text-zinc-400 hover:text-zinc-600 font-black text-xs"
              >
                ×
              </button>
            )}
          ...
          <div className="flex items-center gap-4 text-xs font-semibold">
            ...
            {dossiers.some(d => d.statut === DossierStatus.BLOQUE) && (
              <button 
                ...
                className="bg-red-50 text-red-700 dark:bg-rose-950/20 dark:text-red-400 px-2.5 py-1 rounded-full text-[10px] font-extrabold flex items-center gap-1.5 animate-pulse border border-red-100"
              >
            ...
            <div className="text-right leading-none hidden md:block">
              ...
              <span className="text-zinc-800 dark:text-zinc-300 font-extrabold">{activeRole}</span>
            </div>
          </div>
            ...
            <>
              ...
              {activeTab === "dossiers-liste" && (
                <div className="space-y-4">
                  {/* Filter header bar */}
                  <div className="bg-white dark:bg-neutral-900 border border-slate-200 dark:border-neutral-800 rounded-lg p-4 flex flex-wrap gap-2.5 items-center justify-between shadow-sm">
                    <div>
                      <h3 className="text-sm font-extrabold tracking-tight uppercase font-display text-slate-900 dark:text-neutral-100">Tous les Dossiers Actifs SAV ({filteredDossiers.length})</h3>
                    ...
                    <div className="flex flex-wrap gap-2 text-xs">
                      <select
                        className="p-1.5 px-3 bg-slate-50 border border-slate-200 rounded-md font-semibold dark:text-neutral-100 dark:bg-neutral-950 dark:border-neutral-800 focus:outline-none focus:border-blue-500"
                        value={statusFilter}
                    ...
                    {filteredDossiers.length === 0 ? (
                      ...
                      <div className="overflow-x-auto min-w-full">
                        <table className="w-full text-left border-collapse text-xs font-semibold">
                          <thead>
                            <tr className="border-b border-slate-200 dark:border-neutral-800 uppercase font-bold text-[9px] text-slate-400 bg-slate-50/60 dark:bg-neutral-950/40 p-2 tracking-wider font-display">
                          ...
                          <tbody className="divide-y divide-slate-100 dark:divide-neutral-800">
                            {filteredDossiers.map(doss => (
                              <tr 
                                key={doss.id}
                                className="hover:bg-slate-50/40 dark:hover:bg-neutral-800/40 cursor-pointer transition"
                                onClick={() => setSelectedDossierId(doss.id)}
                              >
                                <td className="py-3 px-4 font-mono font-bold text-slate-900 dark:text-neutral-100">{doss.id}</td>
                                <td className="py-3 px-4 space-y-1">
                                  <div className="font-bold text-slate-800 dark:text-neutral-100 leading-none font-display text-xs">
                                    {doss.clientNom}
                                  </div>
              ...
              {activeTab === "atelier-kanban" && (
                <div className="space-y-4 text-xs font-semibold">
                  <div className="bg-white dark:bg-neutral-900 border border-slate-200 dark:border-neutral-800 p-4 rounded-xl">
                    <h3 className="font-extrabold text-sm uppercase tracking-tight">Tableau Kanban d'Avancement de l'Atelier</h3>
                    <p className="text-xs text-zinc-400">Visualisation dynamique des colonnes de production par statut</p>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-start">
                    ...
                    {/* Received column */}
                    <div className="bg-slate-50 dark:bg-neutral-900 border border-slate-200 dark:border-neutral-800 p-4 rounded-lg space-y-3">
                      ...
                      <div className="space-y-2">
                        {dossiers.filter(d => [DossierStatus.VEHICULE_RECU, DossierStatus.TRAVAUX_PLANIFIES].includes(d.statut)).map(d => (
                          <div 
                            ...
                            className="bg-white dark:bg-neutral-950 p-3 rounded-lg border border-slate-200 dark:border-neutral-800 hover:border-blue-500 hover:shadow-sm transition cursor-pointer space-y-1.5"
                          >
                            <span className="font-mono text-blue-600 dark:text-blue-400 font-extrabold text-[11px]">{d.id}</span>
                            <div className="font-bold text-slate-800 dark:text-zinc-200 leading-tight block truncate font-display">{d.clientNom}</div>
                    ...
                    <div className="bg-sky-50/10 dark:bg-neutral-900 border border-sky-200/40 dark:border-neutral-800 p-4 rounded-lg space-y-3">
                      <span className="font-bold text-xs uppercase text-sky-600 dark:text-sky-400 block border-b pb-1 font-display">
                    ...
                    {/* Blocked column */}
                    <div className="bg-red-50/10 dark:bg-neutral-900 border border-red-200/40 dark:border-neutral-800 p-4 rounded-lg space-y-3">
                      <span className="font-bold text-xs uppercase text-red-600 dark:text-red-400 block border-b pb-1 font-display">
                      ...
                      <div className="space-y-2">
                        {dossiers.filter(d => d.statut === DossierStatus.BLOQUE).map(d => (
                          <div 
                            key={d.id} 
                            onClick={() => setSelectedDossierId(d.id)}
                            className="bg-white dark:bg-neutral-950 p-3 rounded-lg border border-red-200 hover:border-red-500 hover:shadow-sm transition cursor-pointer space-y-1.5"
                          >
                    ...
                    <div className="bg-emerald-50/10 dark:bg-neutral-900 border border-emerald-200/40 dark:border-neutral-800 p-4 rounded-lg space-y-3">
                      <span className="font-bold text-xs uppercase text-emerald-600 dark:text-emerald-400 block border-b pb-1 font-display">
