    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 text-xs font-semibold">
        {performanceKPIs.map((kpi, idx) => {
          ...
          return (
            <div key={idx} className="bg-white dark:bg-neutral-900 p-5 border rounded-2xl shadow-sm flex items-center justify-between">
              <div className="space-y-1.5 text-left">
                <span className="text-zinc-400 text-[11px] block uppercase font-bold tracking-wider leading-none">{kpi.title}</span>
                <span className="text-2xl font-black text-slate-900 dark:text-white block">{kpi.val}</span>
                <span className="text-[10px] text-emerald-600 font-bold bg-emerald-50 dark:bg-emerald-950/40 p-1 px-1.5 rounded">{kpi.change}</span>
              </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-5 bg-white dark:bg-neutral-900 border border-slate-200 dark:border-neutral-800 rounded-xl p-5 shadow-sm space-y-4">
          <div>
            <h3 className="font-bold text-sm text-slate-800 dark:text-neutral-200">PRINCIPAUX FACTEURS DE RETARDS ATELIER</h3>
            ...
            {topBlockers.map((blk, i) => (
              <div key={i} className="space-y-1.5 text-xs text-left font-semibold">
                <div className="flex justify-between items-center text-[11px]">
                  <span className="text-zinc-600 dark:text-zinc-400 line-clamp-1 max-w-[240px]">{blk.reason}</span>
                  <span className="text-blue-600 dark:text-blue-400">{blk.count} dossiers ({blk.percentage}%)</span>
                </div>
                {/* Simulated progress tracking bar */}
                <div className="w-full bg-slate-100 dark:bg-neutral-950 rounded-full h-1.5 overflow-hidden">
        ...
        <div className="lg:col-span-7 bg-white dark:bg-neutral-900 border border-slate-200 dark:border-neutral-800 rounded-xl p-5 shadow-sm">
          <div className="mb-4">
            <h3 className="font-bold text-sm text-slate-800 dark:text-neutral-200">PRODUCTIVITÉ & VALIDATION DES COMPAGNONS</h3>
          ...
          <div className="overflow-x-auto min-w-full">
            <table className="w-full text-left border-collapse text-xs">
              <thead>
                <tr className="border-b text-slate-400 uppercase font-bold text-[9px] bg-slate-50 dark:bg-neutral-800 p-2 text-left tracking-wider">
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-neutral-800">
                {techOutputProductivity.map((tech, i) => (
                  <tr key={i} className="hover:bg-slate-50/50 transition">
                    <td className="py-3 px-3 font-semibold text-slate-800 dark:text-neutral-200">{tech.name}</td>
                    <td className="py-3 px-3">
                      <span className="bg-slate-100 dark:bg-neutral-800 text-zinc-500 font-bold px-1.5 py-0.5 rounded text-[10px] uppercase">
                        {tech.zone}
                      </span>
                    </td>
                    <td className="py-3 px-3 font-bold font-mono">{tech.hoursCharged}</td>
                    <td className="py-3 px-3 font-bold text-blue-600 dark:text-blue-400">{tech.efficacy}</td>
                    <td className="py-3 px-3 text-right">
                      <span className="bg-green-50 dark:bg-green-950 text-green-700 dark:text-green-400 font-bold px-2 py-0.5 rounded text-[10px]">
                        {tech.status}
                      </span>
                    </td>
                  </tr>
