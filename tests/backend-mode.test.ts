...
const ready = resolveBackendRuntimeConfig({ VITE_BACKEND_MODE: "backend-ready" });
...
const incomplete = resolveBackendRuntimeConfig({ VITE_BACKEND_MODE: "backend-enabled" });
...
assert.deepEqual(PUBLIC_BACKEND_ENV_KEYS, ["VITE_SUPABASE_URL", "VITE_SUPABASE_ANON_KEY", "VITE_BACKEND_MODE"]);
