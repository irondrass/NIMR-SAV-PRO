...
test.describe("Lot 6K-F - taches atelier depuis devis et reservation", () => {
  test("Reception depuis base vehicule Description cree un dossier sans taches automatiques", async ({ page }) => {
    ...
    await humanClick(page, page.locator('[data-testid="reception-next"]'));
    await humanFill(page, page.locator('[data-testid="reception-reason"]'), "Controle atelier fictif depuis base vehicule");
    await humanClick(page, page.locator('[data-testid="reception-next"]'));
  ...
  test("Chef Atelier cree une tache, importe un devis PDF et reserve les premiers creneaux", async ({ page }) => {
  ...
  test("Aucun creneau disponible affiche un message clair", async ({ page }) => {
